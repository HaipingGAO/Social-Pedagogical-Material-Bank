...
const ActivityCard = ({ activity, index }: { activity: DetailedActivity, index: number }) => {
  ...
  return (
    <div className={`${bgColor} rounded-[2rem] p-8 shadow-sm ring-1 ring-slate-200 transition-colors`}>
      ...
      {/* Only show the button if there are details to expand */}
      {(activity.supplyList || activity.steps || activity.openEndedQuestions || activity.practicalTips) && (
        <button 
              ...
              <div className="mb-8">
                ...
                <ul className="list-disc list-inside space-y-2 text-slate-600">
                  {activity.openEndedQuestions.map((q, i) => (
                    <li key={i} className="pl-2 italic">{q}</li>
                  ))}
                </ul>
              </div>
            )}
